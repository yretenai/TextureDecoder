...
internal static class Program
{
	...
	private static void WriteGetChannels(IndentedTextWriter writer, string typeName, bool hasRed, bool hasGreen, bool hasBlue, bool hasAlpha)
	{
		...
		if (hasRed && !hasGreen && !hasBlue && !hasAlpha) { // when only red is present, map it to the other channels
            writer.WriteLine("g = b = r = R;");
        } else if (hasGreen && !hasRed && !hasBlue && !hasAlpha) { // when only green is present, map it to the other channels
            writer.WriteLine("r = b = g = G;");
        } else if (hasBlue && !hasRed && !hasGreen && !hasAlpha) { // when only blue is present, map it to the other channels
            writer.WriteLine("r = g = b = B;");
        } else {
